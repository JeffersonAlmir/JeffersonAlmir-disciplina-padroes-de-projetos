...
public class ProntuarioTest {
	...
	public void testCarregarArquivoSemInternacao() {
		...
		try {
			...
			FileService csvProntuario = new CsvProntuarioService(prontuario);
			csvProntuario.carregarProntuario(path);
		} catch (IOException ioException) {
			ioException.printStackTrace();
		}
		...
		assertEquals(10L, procedimentosAgrupados.get(EnumTipoProcedimento.BASICO).longValue());
		assertEquals(2L, procedimentosAgrupados.get(EnumTipoProcedimento.COMUM).longValue());
		assertNull(procedimentosAgrupados.get(EnumTipoProcedimento.AVANCADO));
	}
	...
	public void testCarregarArquivoSemProcedimentos() {
		...
		assertEquals(EnumTipoLeito.APARTAMENTO, internacao.getTipoLeito());
	}
	...
	public void testCarregarArquivoCompleto() {
		...
		assertEquals(EnumTipoLeito.ENFERMARIA, internacao.getTipoLeito());

		Map<EnumTipoProcedimento, Long> procedimentosAgrupados = prontuario.getProcedimentos().stream().collect(
				Collectors.groupingBy(Procedimento::getTipoProcedimento, Collectors.counting()));

				//tentar instanciar um objeto Procedimento e chamar a função getTipoProcedimento
		assertEquals(20L, procedimentosAgrupados.get(EnumTipoProcedimento.BASICO).longValue());
		assertEquals(15L, procedimentosAgrupados.get(EnumTipoProcedimento.AVANCADO).longValue());
		assertNull(procedimentosAgrupados.get(EnumTipoProcedimento.COMUM));
	}
/**
	@Test
